...
from bigness_league_bot.presentation.discord.ticket_message_relay import (
    TicketMessageRelay,
)
...
class TicketsCog(commands.Cog):
    def __init__(self, bot: BignessLeagueBot, store: TicketStateStore) -> None:
        ...
        self.message_relay = TicketMessageRelay(
            bot=bot,
            store=store,
            thread_relay=self.thread_relay,
            participant_messenger=self.participant_messenger,
            command_mirror=self.command_mirror,
            ticket_ai_interactions=self.ticket_ai_interactions,
            send_dm=self._send_dm_with_retry,
        )
    ...
    async def on_message(self, message: discord.Message) -> None:
        await self.message_relay.handle_message(message)
    ...
    async def on_message_edit(
        ...
        await self.message_relay.handle_message_edit(after)

    @commands.Cog.listener()
    async def on_message_delete(self, message: discord.Message) -> None:
        await self.message_relay.handle_message_delete(message)
